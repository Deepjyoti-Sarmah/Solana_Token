...
//STEP 1 - comnnect to solana network
const endpoint = ""; //add your RPC Endpoint
const solanaConnection = new Connection(endpoint);

//STEP 2 - Generate a New Solana Network
...
const secret_array = keypair.secretKey
    ...
    .split(',') //delimit string by comma and convert to an array of string
    .map(value => Number(value)); //convert string values to numbers inside the array

const secret = JSON.stringify(secret_array); //Convert to JSON strign
...
(async() => {
    ...
    try{
        ...
        console.log(`https://explorer.solana.com/tx/${txId}?cluster=devent`);
    }
    catch(err){
        console.log(err);
    }
